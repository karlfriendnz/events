<!--
  An entity record (a Team, Business, School, Family…). Two tabs:
   • Profile — the entity's own fields (from its type), like a person profile.
   • People  — the roster (entity_members): people attached, each with a role.
  The LIVE home for "people connected to an entity" (promoted from /proto).
-->
...
async function load() {
  ...
  const [tk, flds, mem, ppl] = await Promise.all([
    // The type's label + roster slots (own preferred over inherited). Entity types are
    // own-only, so scoping to this org resolves them.
    typesApi.typeByKey([orgId.value!], entity.value.type_key),
    resolveFields(orgId.value!),
    loadMembers(id),
    peopleApi.list(orgId.value!),
  ])
  typeLabel.value = tk?.label ?? entity.value.type_key
  slots.value = tk?.memberSlots ?? []
  fields.value = flds.filter(f => fieldAppliesTo(f, entity.value.type_key))
  members.value = mem
  // Seam is camelCase; personOptions reads snake — map to the shape it expects.
  persons.value = ppl.map(p => ({ id: p.id, first_name: p.firstName, last_name: p.lastName, email: p.email, phone: p.phone }))
  dirty.value = false
  loading.value = false
}
    ...
    <template v-else>
      ...
      <div class="card p-0 overflow-hidden mb-4">
        <div class="px-4 sm:px-5 py-3 flex items-center gap-3">
          <button class="text-gray-400 hover:text-gray-700" @click="router.push('/organisations')"><i class="pi pi-chevron-left" /></button>
      ...
      <div v-show="tab === 'profile'" class="card p-4 sm:p-5 space-y-4 max-w-2xl">
        ...
        <p v-if="!fields.length" class="text-sm text-gray-400">No fields defined for {{ typeLabel }}. Add some in <NuxtLink to="/settings/fields" class="text-primary hover:underline">Settings → Types &amp; fields</NuxtLink>.</p>
