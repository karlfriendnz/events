...
async function load() {
  ...
  loading.value = true
  const [locs, ls, { data: groups }] = await Promise.all([
    loc.loadLocations(),
...
async function addStaff(l: ClubLocation, person: { id: string }) {
  if (!person?.id) return
  await loc.assignStaff(l.id, person.id, pickRole[l.id] || 'staff')
  pick[l.id] = null
  staff.value = await loc.loadLocationStaff()
  toast.add({ severity: 'success', summary: 'Staff assigned', life: 2000 })
}
            ...
            <div class="px-4 sm:px-5 py-3 space-y-1.5">
              ...
              <div class="flex items-center gap-2 pt-1">
                ...
                <Select :model-value="pickRole[l.id] || 'staff'" @update:model-value="(v: string) => pickRole[l.id] = v"
                  :options="LOCATION_STAFF_ROLES" optionLabel="label" optionValue="key" size="small" class="w-44" />
              </div>
            </div>
          </div>
