...
("Dashboard", [
 ...
 ("Finance / outstanding widget", "built",
  ["Outstanding money dashboard tile: unpaid registrations total + count, links to Finances"],
  ["Fee due-date breakdown once the invoice ledger lands"]),
...
("Squads / Classes", [
 ("Locations (multi-site clubs)", "new",
  ["Locations entity + staff access grants (location x sport, null = all)",
   "Global location lens (header switcher) filtering classes/people/attendance/week view/fees/waitlists/allocator/dashboard",
   "Class staff get site access automatically via class assignments",
   "Detail pages kick out like a permission miss when out-of-lens",
   "Term sequences + memberships scopable to one or more locations"],
  ["Sport-gating on remaining lens screens", "Events/bookings lens", "Verify with a real staff login"]),
 ("Drag ordering everywhere", "new",
  ["Classes + memberships boards: drag to reorder (edge bands), drop on programme/tab to move into it",
   "Middle-band drop nests (join umbrella / create-programme dialog)"],
  []),
 ...
 ("Members / Squads reports", "partial",
  ["Classes board columns + saved views + people-tab exports",
   "Classes Reports page: classes/week, hours, spots filled, utilisation — overall + by location + by programme"],
...
("Member portal (self-service)", [
 ("Memberships engine", "new",
  ["Membership = group without a timetable (kind discriminator; roster/fees/waitlist/restrictions/staff for free)",
   "Entitlements: included / % off / $ off per class, programme (dynamic) or event",
   "Programme umbrellas (tiers) + everyone-across-tiers report",
   "Hello Club-parity settings captured: renewal, auto-renewal, anchoring, purchasable-by, payment collection, approval, account credit",
   "Coverage in staff enrol flow (Included in their Senior Membership)",
   "Own URLs/breadcrumbs (/memberships), location scoping, lens-aware creation"],
  ["Renewal/collection/credit engines (settings captured, not enforced)",
   "Member-side purchase + tier-choice registration flow",
   "Public-form coverage + discounted-price math"]),
...
("Settings & platform", [
 ("Deployment smoke tests", "new",
  ["42-check read-only Playwright suite (npm run test:smoke): every core route — 500s, page errors, failing API calls, login bounces",
   "Runs against any TEST_BASE_URL post-deploy; degrades to public subset without creds",
   "Already caught + fixed two silent query bugs (finances, reporting)"],
  ["Wire into the deploy pipeline"]),
 ("Dashboard insight widgets", "new",
  ["12 opt-in widgets: registrations/week, sign-up gaps, outstanding money, utilisation, waitlists (enrollable today), locations, season pulse, attendance, staff coverage, memberships, retention, birthdays",
   "Content block (rich text, background, image, buttons) for member dashboards",
   "Self-loading widget architecture: any feature ships a dashboard card with one file"],
  []),
