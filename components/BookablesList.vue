            ...
            <div class="flex items-center justify-between mt-2">
              <div class="flex items-center gap-1.5">
                <span v-if="item.status === 'ACTIVE'"
                  ...
                  :class="bookedIds.has(item.id) ? 'bg-amber-50 text-amber-700 border-amber-200' : partialIds.has(item.id) ? 'bg-indigo-50 text-indigo-600 border-indigo-200' : 'bg-green-50 text-green-700 border-green-200'">
...
const visibleTabs = computed(() =>
  tabs.filter(t => t.value !== 'ARCHIVED' || archivedCount.value > 0),
)
...
async function openCreate() {
  if (activeTab.value === 'VENUE') {
    await navigateTo('/bookables/new')
    return
  }
  form.value = { name: '', internal_name: '', item_category: '', location: '', max_concurrent: 1, is_public: false, description: '' }
...
onMounted(() => { load(); loadArchivedCount() })
</script>
