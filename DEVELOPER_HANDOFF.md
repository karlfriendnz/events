...
6. [Page-by-page guide](#6-page-by-page-guide)
   ...
   - 6.18b [Club setup `/settings/modules`](#618b-club-setup-settingsmodules)
   - 6.18c [Xero `/settings/xero`](#618c-xero-settingsxero)
...
- `227_group_form.sql` + `229_form_targets.sql` — **Public registration — the easy way for people to sign up.** Every class (group) can now have its own **public signup page** at `/r/group/{id}`: a parent opens a link (or scans a **QR code**), fills in the form, picks **how they want to pay** (the group's fee options), and they're enrolled — no login needed. If the class is **full**, the page offers the *equivalent classes with space* (the ones sharing its waitlist — "Thursday's full, but Friday has 3 spaces"), and submitting joins the **waitlist** instead. Staff set this up from a new **"Public registration" dialog** on the group page (the "Ready for signup" strip now has **Copy link** and **Share** buttons): they can **design the form in the full form designer** — the *same* rich builder events use, now at `/groups/{id}/form` — **quick-create** a standard form in one click, or connect a form they already built. The readiness strip also warns when a class has *no registration form* yet. Separately, a **form is now first-class and connectable**: the **Forms** page (in **Settings → Forms**) gives each form — and the form editor's toolbar — a **Connect** button opening a **tree of the whole class hierarchy** — tick a **programme** (a Code) to offer *every* class inside it, including ones added later, or tick individual classes (`registration_form_targets`, type `code`/`group`). **Creating a form is a proper 3-step wizard** (`/forms/new`: name & who registers → connect classes, inline → land in the designer; nothing is saved until the final "Create & design" click, so abandoning it leaves no junk rows). Its public page `/r/form/{id}` then shows a **"Choose your class(es)"** step — classes grouped under their programme headings, live space counts, per-class prices, full classes offer their waitlist, classes from ended terms are hidden — and one submit enrols each registrant into everything they picked. Events got a matching **"Copy public link"** button on the Invitees tab. *(Columns/tables: `member_groups.form_id`, `registration_form_targets`; the public page also now renders forms built in the simple /forms builder, not just designer forms. Note: migration 228 is the Xero work from a parallel session.)* **Still to build:** payment *processing* on submission (method is captured, nothing is charged), a confirmation email to the registrant, and a public "browse everything you can register for" landing page.
...
- `201_group_term_lineage.sql` — **Term rollover — starting each term fresh.** A group now belongs to **one term**. When a term ends, that group becomes read-only **history** (it shows an amber "this term has ended" banner and you can't edit it) — nothing is deleted, so last term's roster and results stay on record. To run the next term you use the new **Roll over groups** screen (`/groups/rollover`, reached from the "Roll over a term" link on the Groups page): pick the term you're rolling **from** and the term you're rolling **into**, then for each group tick whether to include it, rename it if you like, and choose what carries over — **for both coaches and members** you can **roll everyone over**, **start empty** (they'll re-register via the form), or **pick just some**. Clicking "Create groups" makes a fresh copy of each chosen group for the new term (keeping its training times, membership plans and fee), and nested groups keep their structure. It's safe to run twice — anything already created for that term is skipped. The Groups page has a **term filter** so you can flick between terms (past terms show their history). When someone signs up through a registration form, they're now recorded as joining **that group's term**. *(Behind the scenes: `member_groups` gains `term_id`, `lineage_id` — the thread linking "Step 2 Tuesday" across every term — and `rolled_from_group_id`.)* **New: each term now has a sign-up window** — on Settings → Terms & memberships every term row has "Sign-up opens" and "Sign-up closes" dates controlling when members can register for that term's classes (leave them blank to open right away and close when the term ends). The registration form itself never carries dates — the term does — so two terms can be open at once (late joiners for the current term + re-enrolment for the next). **New: the dashboard now reminds the club to roll over** — when the running term is inside its last 3 weeks (or ended within the last 45 days) and some classes have no copy in the next term yet, a solid-yellow banner appears at the top of the dashboard ("Term 3 ends in 12 days — 18 of 19 classes haven't been rolled into Term 4 yet") with a "Roll over now" button straight into the rollover screen, or "Set up terms" when the next term hasn't been created; "Remind me later" snoozes it for 3 days. **New: the Term set-up wizard** — the banner's button now opens a guided wizard (/groups/term-wizard) that walks the club through the whole thing with no help needed: it suggests the next term's name and dates (create it or pick an existing one), asks when sign-ups should open (e.g. "2 weeks before the term starts"), whether to bring classes over (keep everyone / keep coaches and let members re-register / classes only / none), sorts fees (keep each class's fees, add one fee to every class, or later), shows a plain-English summary of exactly what will happen, then does it all in one click and confirms what it did. The wizard saves the term as soon as you finish step 1, lets you rename classes and choose per-class whether staff and members carry, shows every carried fee (with its Xero code) for editing or bulk-adding across many classes at once, and — new — **creates the whole term's training sessions automatically** (each class's weekly times become real events with rosters pre-loaded for attendance), so a rolled-over term comes out fully scheduled. Classes keep their programmes, training times, fees, photos and public registration links when they roll. **Not yet built:** automatically generating the new term's training events (you still press "Create training events" on each new group).
...
### 6.18c Xero `/settings/xero`

- **URL:** `/settings/xero`
- **Source:** `pages/settings/xero.vue`, `server/utils/xero.ts`, `server/api/xero/*`

**What this page is for**
Connects the club's own Xero accounting organisation to FriendlyManager, so that (in the next phase) fees, invoices and payments flow into the club's real books automatically.

**How it works**
- **Connect** — the admin clicks "Connect to Xero", signs into Xero, and approves access. FriendlyManager remembers which Xero organisation was connected and shows its name with a green "Connected" chip.
- **"Finish your Xero setup"** — a numbered, step-by-step flow where each step only appears once the previous one is done (the admin always sees exactly one thing to do next): **Step 1** pick the bank account payments land in (with a link to add one in Xero if there are none) → **Step 2** pick the default tax rate → **Step 3** build an "Accounts you use" list — the income accounts fees post to, each given a club-friendly name, with one marked **Default** (the fallback for anything without its own account). The **Save setup** button only appears once all three steps are complete.
- **Tracking categories** — if the club uses Xero tracking categories (e.g. "Location: HBC / Albany"), each "Accounts you use" entry shows a small picker per category. Attach an option and every fee posted with that account is tracked with it in Xero. (Same behaviour as the old system: the tracking rides along with the fee's account.)
- **Disconnect** — removes the connection (and tells Xero to revoke access). Reconnecting to the *same* Xero organisation later resumes cleanly.

**Account-code lookup everywhere**
Once Xero is connected, every "Account code" / "GL" box across the platform (event fee line items, group fee line items, the bulk group-fees dialog, form financial rules) grows a small book icon. Clicking it opens a search of the club's named "Accounts you use" list and the full income chart — pick one and the code is filled in. Entries carrying tracking show a tag chip in the box instead of raw text. Without Xero connected, the boxes stay plain free-text (nothing changes). Component: `<XeroAccountInput>`; catalogue: `useXeroAccounts()`.

**Rules (learned from the two working integrations we studied — PupManager and legacy FM)**
- A club can never be silently switched to a *different* Xero organisation — reconnect only accepts the same one.
- Xero replaces the "refresh token" every time we use it; we always store the new one, or the connection would silently die.
- Tokens live server-side only; the settings page never sees them.
- The actual syncing (members → contacts, fees → invoices, payments both ways) is **not built yet** — it needs the invoice/payment ledger first. The error-queue table (`xero_sync_errors`) already exists for it.

**Setup needed** — a Xero developer app's `XERO_CLIENT_ID` + `XERO_CLIENT_SECRET` in the environment, with `<site>/api/xero/callback` registered as the redirect URI in the Xero app.

**Database** — `xero_connections` (one row per club: tenant, tokens, account mapping) + `xero_sync_errors`. Migration 228.

---

### 6.18b Club setup `/settings/modules`

- **URL:** `/settings/modules`
- **Source:** `pages/settings/modules.vue`, `composables/useOrgModules.ts`

**What this page is for**
Every club is different — a gym club might not take bookings, a tennis club might not run classes. This screen lets a club **turn the different parts of the system on or off** so the menu only shows what they actually use.

**How it works**
- One row per module (Classes & groups, Events, Bookings, Attendance, Fees & finances, Mailer, Resources, Assets, Mobile app, Programme, GNZ, FM Invoices), each with an icon, a plain-English description and an on/off switch.
- Flipping a switch saves straight away, and the left menu (and the mobile bottom bar and "More" sheet) updates **instantly** — no refresh needed.
- Dashboard and People are shown in a separate "Always on" card with a lock — every club has those.
- A new club starts with **everything on** (no saved choice = all on). A "Turn everything on" link resets back to that.

**Rules**
- Turning a module off only hides it from the menus — typing the web address directly still opens the page (route blocking is a possible follow-up).
- Only admins (settings permission) can change this.

**Database** — `organisations.enabled_modules` (a list of the module keys that are on; empty/null = everything on). Migration 226.
